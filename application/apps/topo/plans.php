...
if (count($resultat)==0){
    ...
    $ch= "https://".$_SERVER['HTTP_HOST']."/apps/topo/topo.php?dess=../../doc_commune/".$insee."/dwf/".strtolower($resultat[0]['local1'])."/".strtolower($resultat[0]['fichier']);
    include($ch);
}else{
      ...
      for ($j=0;$j<count($resultat);$j++){
	     echo '<tr><td><a href="https://'.$_SERVER['HTTP_HOST'].'/apps/topo/topo.php?dess=../../doc_commune/'.$insee.'/dwf/'.strtolower($resultat[$j]['local1']).'/'.strtolower($resultat[$j]['fichier']).'">'.$resultat[$j]['fichier'].'.dwg</a></td>';
