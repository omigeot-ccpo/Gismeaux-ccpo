...
if (isset($_GET["obj_keys"])){
   ...
   while($_POST["parcelle"][$po]!=""){
	 ...
	 $wh.="(a.identifian='".$codeins.strtoupper($_POST["section"][$po]).$b1."' and Intersects(a.the_geom,b.the_geom)) or ";
     $po++;
   }
}else{
...
$table=array("zonebruit","ppri2","zoneboise","lisiere","ppmh","sidobre","emplreserve","z_archeo","siteinscrit","servradioelectrik","zononaedificandi","alignement","halage","cimetiere","lotissement");
     ...
     $cinon=0;
	 for($j=0;$j<=$nb-1;$j++){
       $q="select Intersects(a.the_geom,b.the_geom) as inters, b.gid as gid, area(intersection(a.the_geom,b.the_geom)) as aire from cadastre.parcelle as a ,geotest.$table[$j] as b where ".$wh.";";
...
$result = $DB->tab_result($toto);
for ($i=0; $i<count($result); $i++){
          if($result[$i][2]>2 and ($result[$i][0]=="12" or $result[$i][0]=="13")){
		  $simple="okcoche.png";
          $beneficaire="AFTRP";   
          }
}
...
if($idinond=="true"){
    ...
    $q="select distinct(nom_zr) as nom_z, code_zr from geotest.ppri2 where gid in (".$zoninon.") and code_zr <> '0' order by code_zr asc;";
