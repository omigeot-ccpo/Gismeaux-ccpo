...
$q="select * from general.quartier where gid=".$_GET['obj_keys'];
$rq = $DB->tab_result($q);
$sql2="select * from urba.program where distance(the_geom,'".$rq[0]['the_geom']."')=0 and date_livraison is null order by annee";
$rsql2=$DB->tab_result($sql2);
$sql3="select fill from admin_svg.col_theme where idappthe=(select idappthe from admin_svg.appthe where idtheme=(select idtheme from admin_svg.theme where libelle_them='quartier')  and idapplication='18') and valeur_texte='".$rq[0]['nom']."'";
...
$pdf->SetFont('','B',13);
$pdf->Setxy(15,20);
$pdf->Write(6,'PrÈvisions - Livraisons de programmes de logements pour le quartier '.$rq[0]['nom']);
$pdf->Setxy(15,40);
$pdf->SetFillColor($rgb[0],$rgb[1],$rgb[2]);//prendre couleur du thËme
$pdf->SetFont('','',7);
$pdf->cell(20,10,'NumÈro',1,0,'C',1);
$pdf->cell(60,10,'Programme',1,0,'C',1);
$pdf->cell(30,10,'Nombre de logts',1,0,'C',1);
$pdf->cell(30,10,'Collectif/Individuel',1,0,'C',1);
$pdf->cell(30,10,'Logts sociaux',1,0,'C',1);
$pdf->ln();
...
for ($i=0;$i<count($rsql2);$i++){
	if ($an_ini!=$rsql2[$i]['annee']){
		...
		$pdf->cell(60,8,'',1,0,'',1);
		$pdf->cell(30,8,$tot_lgt,1,0,'C',1);
		$pdf->cell(30,8,$tot_coll."/".$tot_ind,1,0,'C',1);
		$pdf->cell(30,8,$tot_soc,1,0,'c',1);
		$tot_lgt=0;$tot_coll=0;$tot_ind=0;$tot_soc=0;
		$pdf->ln();
		$pdf->ln();
		$y_ini=$pdf->gety();
	}
	$pdf->cell(20,8,$rsql2[$i]['num'],1,0,'C',0);
	$pdf->cell(60,8,$rsql2[$i]['nom'],1,0,'',0);
	$pdf->cell(30,8,$rsql2[$i]['nbre_lgt'],1,0,'C',0);$tot_lgt=$tot_lgt+$rsql2[$i]['nbre_lgt'];
	$pdf->cell(30,8,$rsql2[$i]['nbre_coll']."/".$rsql2[$i]['nbre_ind'],1,0,'C',0);$tot_coll=$tot_coll+$rsql2[$i]['nbre_coll'];$tot_ind=$tot_ind+$rsql2[$i]['nbre_ind'];
	$pdf->cell(30,8,$rsql2[$i]['logt_sociaux'],1,0,'c',0);$tot_soc=$tot_soc+$rsql2[$i]['logt_sociaux'];
	$pdf->ln();
}
$yav=$pdf->gety();
$yht=$yav-$y_ini;
