...
FROM
	urba.program program 
		INNER JOIN scolaire.contour2007 ccq 
		ON distance(program.the_geom , ccq.the_geom )=0 
WHERE
	(program.date_livraison IS NULL)  and ccq.type_ecole='primaire'
GROUP BY
	ccq.nom,program.annee) as foo group by annee order by annee asc";
$rsq=$DB->tab_result($sq);
$q="select * from scolaire.contour2007 where type_ecole='primaire' order by nom";
$rq = $DB->tab_result($q);
...
$pdf->SetFont('','B',13);
$pdf->Setxy(15,20);
$pdf->Write(6,'PrÈvisions - Livraisons de programmes de logements (selon ecole primaire)');
//dÈfinition des variables de prÈsentation
$ht=6; //hauteur cellule
$lnum=5; //largeur de la colonne num
$lprog=32; //largeur colonne programme
$llog=7; //largeur de la colonne nb logements
$lcolind=7; //largeur de la colonne collectif/individuel
$lsocio=6; //largeur de la colonne logement sociaux
$lcol=$lnum+$lprog+$llog+$lcolind+$lsocio; //largeur de la colonne
...
for ($o=0;$o<count($rq);$o++){
	...
	//$sql3="select fill from admin_svg.col_theme where idappthe=(select idappthe from admin_svg.appthe where idtheme=(select idtheme from admin_svg.theme where libelle_them='nouveau perimetre')) and valeur_texte='".$rq[$o]['nom']."'";
	$sql3="select fill from admin_svg.col_theme where idappthe=50 and valeur_texte='".$rq[$o]['nom']."'";
	...
	$pdf->SetLeftMargin(11+($lcol*$q));
	$pdf->SetFont('','',$ht+2);
	$pdf->Setxy(11+($lcol*$q),30);
	$pdf->cell($lcol,$ht+2,$rq[$o]['nom'],1,0,'C',0);
	$pdf->ln();
	$pdf->SetFillColor($rgb[0],$rgb[1],$rgb[2]);//prendre couleur du thËme
	$pdf->SetFont('','',$ht-1);
	$pdf->cell($lnum,$ht+2,'Num',1,0,'C',1);
	$pdf->cell($lprog,$ht+2,'Programme',1,0,'C',1);
	$pdf->cell($llog,$ht+2,'Nb logts',1,0,'C',1);
	$pdf->cell($lcolind,$ht+2,'Coll/Ind',1,0,'C',1);
	$pdf->cell($lsocio,$ht+2,'Sociaux',1,0,'C',1);
	$pdf->ln();
	...
	for ($j=0;$j<count($rsq);$j++){
		if ($rsq[$j]['annee']!=$rsql2[$k]['annee']){
			for ($l=0;$l<$rsq[$j]['ht'];$l++){
				$pdf->cell($lnum,$ht,'',1,0,'C',0);
				$pdf->cell($lprog,$ht,'',1,0,'',0);
				$pdf->cell($llog,$ht,'',1,0,'C',0);
				$pdf->cell($lcolind,$ht,'',1,0,'C',0);
				$pdf->cell($lsocio,$ht,'',1,0,'c',0);
				$pdf->ln();
			}
			$pdf->cell($lnum,$ht,'',1,0,'C',1);
			$pdf->cell($lprog,$ht,'',1,0,'',1);
			$pdf->cell($llog,$ht,'',1,0,'C',1);
			$pdf->cell($lcolind,$ht,'',1,0,'C',1);
			$pdf->cell($lsocio,$ht,'',1,0,'c',1);
			$pdf->ln();
			$pdf->ln();
		}else{
			for ($i=$k;$i<count($rsql2);$i++){
				if ($rsq[$j]['annee']!=$rsql2[$i]['annee']){break;}
				$pdf->cell($lnum,$ht,$rsql2[$i]['num'],1,0,'C',0);
				$pdf->cell($lprog,$ht,$rsql2[$i]['nom'],1,0,'',0);
				$pdf->cell($llog,$ht,$rsql2[$i]['nbre_lgt'],1,0,'C',0);$tot_lgt=$tot_lgt+$rsql2[$i]['nbre_lgt'];
				$pdf->cell($lcolind,$ht,$rsql2[$i]['nbre_coll']."/".$rsql2[$i]['nbre_ind'],1,0,'C',0);
				$tot_coll=$tot_coll+$rsql2[$i]['nbre_coll'];$tot_ind=$tot_ind+$rsql2[$i]['nbre_ind'];
				$pdf->cell($lsocio,$ht,$rsql2[$i]['logt_sociaux'],1,0,'c',0);$tot_soc=$tot_soc+$rsql2[$i]['logt_sociaux'];
				$pdf->ln();
			}
			...
			$pdf->cell($lnum,$ht,'',1,0,'C',1);
			$pdf->cell($lprog,$ht,'',1,0,'',1);
			$pdf->cell($llog,$ht,$tot_lgt,1,0,'C',1);
			$pdf->cell($lcolind,$ht,$tot_coll.'/'.$tot_ind,1,0,'C',1);
			$pdf->cell($lsocio,$ht,$tot_soc,1,0,'c',1);
			$pdf->ln();
			$pdf->ln();
