...
if ($_GET['raster']!='')
  {
    ...
    error_reporting (1);
    $sql_app="select supp_chr_spec(libelle_appli) as libelle_appli from admin_svg.application where idapplication=".$appli;
    ...
    if(substr($insee, -3)=='000')
      {
	$url=get_root_url()."/cgi-bin/mapserv?map=".$fs_root."capm/".$application.".map&map_imagetype=jpeg&insee=".substr($insee,0,3)."&layer=".$raster."&minx=".$xm."&miny=".$ym."&maxx=".$xma."&maxy=".$yma."&mapsize=1240%201040&parce=('')";
      }
    else
      {
	$url=get_root_url()."/cgi-bin/mapserv?map=".$fs_root."capm/".$application.".map&map_imagetype=jpeg&insee=".$insee."&layer=".$raster."&minx=".$xm."&miny=".$ym."&maxx=".$xma."&maxy=".$yma."&mapsize=1240%201040&parce=('')";
      }
    $contenu=file($url);
	...
	if(in_array($cou[$l]['idappthe'],$id))
	  {
	    ...
	    $f=substr($f,0,-1)." from ".$cou[$l]['schema'].".".$cou[$l]['tabl'];
            ...
            if (substr($insee, -3) == "000" || $cou[$l]['schema']=="bd_topo")
	      {
		$f.=" where ".$geometrie." && box'(".$xm.",".$ym.",".$xma.",".$yma.")'";
	      }
	    else
	      {
                $f.=" where (code_insee like '".$insee."%' or code_insee is null) and ".$geometrie." && box'(".$xm.",".$ym.",".$xma.",".$yma.")'";
	      }
	    $j="select * from admin_svg.col_where where idtheme='".$cou[$l]['idtheme']."'";
	    $whr=$DB->tab_result($j);
	    if (count($whr)>0)
	      {
		$f.=" and ".str_replace("VALEUR","'".$insee."'",$whr[0]['clause']);
	      }
	    $type_geo="";	
	    if($geometrie!="")
	      {
...
if ($_GET['raster']!='')
  {
    $ch_image=" ".$fs_root."tmp/".$image.".jpg";
  }
$da=date("His");
...
header("Location: ".get_root_url()."/tmp/carte".$da.".zip");
?> 
