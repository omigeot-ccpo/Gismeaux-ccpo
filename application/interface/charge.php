...
$serv="127.0.0.1";
	...
	$url="http://".$serv."/cgi-bin/mapserv?map=".$fs_root."capm/".$application.".map&insee=".substr($_SESSION['profil']->insee,0,3)."&layer=".$raster."&minx=".$xm."&miny=".$ym."&maxx=".$xma."&maxy=".$yma."&mapsize=1200%20840&parce=('')".$extra_url;
	}
	else
	{
		...
		if ($cou[0]['partiel']==1)
		{
            if($cou[0]['schema']!="bd_topo")
			{
				if (substr($_SESSION['profil']->insee, -3) == "000")
                	...
                	$f.=" where (code_insee like '".substr($_SESSION['profil']->insee,0,3)."%' or code_insee is null) and ".$geometrie." && box'($xm,$ym,$xma,$yma)'";
            		}
				else
					{
