...
if ( $_GET["type"]=='raster')
  {
    ...
    if(substr($_SESSION['code_insee'], -3)=='000')
      {
	$url=get_root_url()."/mapserv.php?map=".$fs_root."/capm/".$application.".map&insee=".substr($_SESSION['code_insee'],0,3)."&layer[]=".$raster."&minx=".$xm."&miny=".$ym."&maxx=".$xma."&maxy=".$yma."&mapsize=1240%201040&parce=('')";
      }
    else
      {
	$url=get_root_url()."/mapserv.php?map=".$fs_root."/capm/".$application.".map&insee=".$_SESSION['code_insee']."&layer[]=".$raster."&minx=".$xm."&miny=".$ym."&maxx=".$xma."&maxy=".$yma."&mapsize=1240%201040&parce=('')";
      }
    $contenu=file($url);
	 ...
	 if($cou[0]['schema']!="bd_topo")
	   {
	     if (substr($_SESSION['code_insee'], -3) == "000")
	       {
		 $f.=" where (code_insee like '".substr($_SESSION['code_insee'],0,3)."%' or code_insee is null) and ".$geometrie." && box'($xm,$ym,$xma,$yma)'";
	       }
	     else
	       {		 
