...
function impression()
{
	...
	var url="http://" + serveur + "/interface/preprint.php?nav="+nav+"&raster="+couches+"&x="+ xnulcorner + "&y=" + ynulcorner + "&lar=" + nWidth  + "&hau=" + nHeight+ "&zoom=" + zoomVal  + "&xini=" + xini + "&yini=" + yini +"&nav="+nav+"&sessionname=" + sessionname + "&sessionid=" + sessionid +"&echini="+(nWidth*3300/620)+"&parce=('"+placid+"'"+ajo;
var param="_blank";
var zer="liprint";
ouvrepage(url,param,zer)
couches="";
...
function enregistre()
{
	...
	nom=sessionid + plus;
	enregistr(nom);
	var url="http://" + serveur + "/interface/ouv_zip.php?image=" +image + "&nom=" + nom + '&sessionname=' + sessionname + '&sessionid=' + sessionid;
		var zer="lienre";
var param="";
ouvrepage(url,param,zer)
	
}
function enregistr(x)
{
	nomval=x;
	animation()
	var nb = layer.length;
