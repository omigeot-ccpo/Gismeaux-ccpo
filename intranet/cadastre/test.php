...
if ($vmysql=='mysql') {
	...
	$lk=mysql_pconnect($bdmysql,$umysql,$pswmysql)
		or die ("Connexion impossible à MYSQL");
	if (mysql_create_db("cadastre")){
		/* Puisque création de la base, création des tables */
		...
		print ("Table des dépendances créée\n"); ?> <BR> <?php 
		...
		print ("Table de description créée\n"); ?> <BR> <?php 
		...
		print ("Table description des habitations créée\n"); ?> <BR> <?php 
		...
		print ("Table professionnelle créée\n"); ?> <BR> <?php 
		...
		print ("Table de subdivision des locaux créée\n"); ?> <BR> <?php 
		...
		print ("Table de la taxation créée\n"); ?> <BR> <?php 
		...
		print ("Table éxonération parcelle créée\n"); ?> <BR> <?php 
		...
		print ("Table de subdivision des parcelles créée\n"); ?> <BR> <?php 
		...
		print ("Table de taxation des parcelles créée\n"); ?> <BR> <?php 
		...
		print ("Table des parcelles créée\n"); ?> <BR> <?php 
		...
		print ("Table des propriétaires créée\n"); ?> <BR> <?php 
		...
		print ("Table des communes créée\n"); ?> <BR> <?php 
	}else{
		...
		function rename_table($name_tb,$ann){
			...
			if (mysql_query("create table ".substr($name_tb,0,-2).$dx." as select * from ".$name_tb.";")){
				print ("Table ".$name_tb." renommée");
				mysql_query("delete from ".$name_tb.";");
				echo "Table ".$name_tb." renommée en ".substr($name_tb,0,-2).$dx." et table ".$name_tb." créée\n"; ?> <BR> <?php 
			}else{
...
$feror=fopen($ferr,"a");
if ($nbat!=""){
	$fp=fopen($nbat,"r"); /* Ouverture du fichier du non bâti et chargement des tables*/
	if ($voracle=='oracle') {
		$ctab=fopen($rep_f."cree_tab.sql",a);				/* Intégration dans le script sql de l'insertion des données */
		fwrite($ctab,"start ".$rep_f."parcel.ora;\n");		/* Données de la table parcelle */
		fwrite($ctab, "commit;\n");
		fwrite($ctab,"start ".$rep_f."psubd.ora;\n");			/* Données de la table subdivision */
		fwrite($ctab, "commit;\n");
		fwrite($ctab,"start ".$rep_f."pexo.ora;\n");			/* Données de la table exonération */
		fwrite($ctab, "commit;\n");
		fwrite($ctab,"start ".$rep_f."ptaxat.ora;\n");		/* Données de la table taxation */
		fwrite($ctab, "commit;\n");
		fclose($ctab);
		$parc_ora=fopen($rep_f."parcel.ora",w);				/* Ouverture en écriture des fichiers de données */
	...
	/* Affiche à l'écran le traitement déjà effectué */
		if ($section!=$ccosec) {
		$section=$ccosec;
		?><br>Traitement du non bâtie de la section : <?php echo $ccosec; }
		$linfo=fgets($fp);
	}
fclose($fp);
if ($voracle=='oracle') {
...
/* set_time_limit(120); */
if ($bat !=""){
	$fp=fopen($bat,"r"); /* Ouverture du fichier du bâti et chargement des tables*/
	if ($voracle=='oracle') {
		$ctab=fopen($rep_f."cree_tab.sql",a);				/* Intégration dans le script sql de l'insertion des données */
		fwrite($ctab,"start ".$rep_f."batidgi.ora;\n");		/* Données de la table batidgi */
		fwrite($ctab, "commit;");
		fwrite($ctab,"start ".$rep_f."bdes.ora;\n");			/* Données de la table description */
		fwrite($ctab, "commit;");
		fwrite($ctab,"start ".$rep_f."bsub.ora;\n");			/* Données de la table subdivision */
		fwrite($ctab, "commit;");
		fwrite($ctab,"start ".$rep_f."bexo.ora;\n");			/* Données de la table éxonération */
		fwrite($ctab, "commit;");
		fwrite($ctab,"start ".$rep_f."btax.ora;\n");			/* Données de la table taxation */
		fwrite($ctab, "commit;");
		fwrite($ctab,"start ".$rep_f."bhab.ora;\n");			/* Données de la table descriptif d'habitation */
		fwrite($ctab, "commit;");
		fwrite($ctab,"start ".$rep_f."bpro.ora;\n");			/* Données de la table descriptif de local professionnel */
		fwrite($ctab, "commit;");
		fwrite($ctab,"start ".$rep_f."bdep.ora;\n");			/* Données de la table descriptif de dépendance */
		fwrite($ctab, "commit;");
		fclose($ctab);
		$batidgi_ora=fopen($rep_f."batidgi.ora",w);			/* Ouverture en écriture des fichiers de données */
	...
	fclose($fp);
	if ($voracle=='oracle') {
		fclose($batidgi_ora);			/* Fermeture des fichiers de données */
		fclose($bdes_ora);			
		fclose($bsubd_ora);			
		fclose($bexo_ora);			
		fclose($btax_ora);			
...
set_time_limit(20);
if ($prop !=""){
	$fp=fopen($prop,"r"); /* Ouverture du fichier du propriétaire et chargement des tables*/
	if ($voracle=='oracle') {
		$ctab=fopen($rep_f."cree_tab.sql",a);				/* Intégration dans le script sql de l'insertion des données */
		fwrite($ctab,"start ".$rep_f."propri.ora;\n");		/* Données de la table Propriétaire */
		fwrite($ctab, "commit;\n");
		$prop_ora=fopen($rep_f."propri.ora",w);
	}
	$section=" ";$i=0;$j=0;$k=0;$l=0;$v="','";
	echo "Traitement des propriétaires en cours!";
	while (!feof($fp)){
		set_time_limit(2);
		$linfo=fgets($fp);
		$code_commune=substr($linfo,0,6);
	...
	fclose($fp);
	if ($voracle=='oracle') {
		fclose($prop_ora);			/* Fermeture des fichiers de données */
	}
}
set_time_limit(20);
if ($fant !=""){
	...
	while (!feof($fp)){
		...
		if ($code!=""){
			if ($nat=="AER "){$libel="Aérodrome ";}
			elseif ($nat=="AERG"){$libel="Aérogare ";}
			elseif ($nat=="AGL "){$libel="Agglomération ";}
			elseif ($nat=="ALL "){$libel="Allée ";}
			...
			elseif ($nat=="CARE"){$libel="Carrière ";}
			...
			elseif ($nat=="DEVI"){$libel="Déviation ";}
			...
			elseif ($nat=="PTA "){$libel="Petite allée ";}
	...
	fclose($fp);
	if ($voracle=='oracle') {
		fclose($voie_ora);			/* Fermeture des fichiers de données */
	}
}
fclose($feror);
	if ($voracle=='oracle') {
		echo "Lancez SQLPLUS puis la commande :'start ".$rep_f."cree_tab.sql; ' pour réaliser la création et l'insertion des données dans Oracle.";
	}
